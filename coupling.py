...
class coupling:
    def __init__(self, B, S, T, eigenvectors, q_vector, omega_q, R_vectors, L_vectors):
        ...
        self.D_d1, self.G_d1,self.disp1 = read_files.read_d1()
        self.compute_dH_dx()

        self.D_d2, self.G_d2, self.disp2 = read_files.read_d2()
    ...
    def compute_V_alpha_beta_q(self, Nq1, Nq2, Nomega1, Nomega2):
        ...
        # Get precomputed values with correct indexing
        prefactor1 = self.prefactor[Nq1, Nomega1, :]  # Shape: (N,)
        prefactor2 = self.prefactor[Nq2, Nomega2, :]  # Shape: (N,)
        exp1 = self.exp[Nq1, :]  # Shape: (N,)
        exp2 = self.exp[Nq2, :]  # Shape: (N,)
        L1 = self.L_vectors_mol[Nq1, Nomega1, :, :]  # Shape: (N, hdim)
        L2 = self.L_vectors_mol[Nq2, Nomega2, :, :]  # Shape: (N, hdim)
    
        
        # OPTIMIZATION 2: Vectorized computation of all factors at once
        combined_factors = (prefactor1[self.valid_i] * prefactor2[self.valid_j] * 
                           exp1[self.valid_i] * exp2[self.valid_j])
        
        # OPTIMIZATION 3: Advanced indexing to extract valid L vectors
        L1_valid = L1[self.valid_i]  # shape: (num_pairs, hdim)
        L2_valid = L2[self.valid_j]  # shape: (num_pairs, hdim)
        
        # OPTIMIZATION 4: Extract valid H2 tensor elements
        H2_valid = self.dH2_dxdx[self.valid_i, self.valid_j]  # shape: (num_pairs, hdim, hdim, hdim, hdim)
        
        # OPTIMIZATION 5: Ultra-efficient einsum with optimize=True
        # This single line replaces all the nested loops
        V_alpha_beta = np.einsum('p,pa,pb,pabmn->mn', 
                                  combined_factors, L1_valid, L2_valid, H2_valid,
                                  optimize=True)
        
        # OPTIMIZATION 6: In-place Hermiticity operation
